...
def añadirCoche(request):
    ...
    if request.method == 'POST':
        cocheform = cocheForm(data=request.POST)
        lugarform = lugarForm(data=request.POST)
        if cocheform.is_valid():
                # Create Comment object but don't save to database yet
                new_coche = cocheform.save()
                # Assign the current coche to the comment
                new_coche.coche = model
                new_coche.usuario = usuarios
                # Save the comment to the database
                new_coche.save()
    else:            
        coche_form = CommentForm()
    return render(request, template_name, {'coche_form': coche_form})
